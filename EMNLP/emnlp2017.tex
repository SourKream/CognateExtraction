...
In this paper, we proposed to use an end-to-end recurrent neural network based model with the need for explicit string similarity feature engineering. LSTM (Long Short Term Memory) networks are being used in a extensive range of NLP tasks to build end-to-end systems. LSTM networks being a form of recurrent networks, fit more naturally to natural language as compared to convolutional networks.  


, there have been several attempts to calthough there have been advances in automatic cognate information.


applied to NLP tasks, like sentence alignment in bitexts and improving statistical machine translation models.\texttt{Need some citations here}


The high level of linguistic diversity in South Asia poses the challenge of building lexical resources across the languages from these regions. An important source for assisting the creation and sharing of lexical resources between languages is information about cognate words. 



We do an extensive analysis of our trained models to try to understand the kinds of features it learns to recognize that are important for making the cognate judgment. We also specifically try to apply the model to the domain on Hindi-Marathi, by applying it to a large unlabeled corpus of aligned texts to find cognate pairs.
