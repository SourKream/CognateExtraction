...
for fold in range(numFolds):
	...
	numMeaningsPerFold = len(words)/numFolds
	testMeanings = set(words.keys()[fold*numMeaningsPerFold:(fold+1)*numMeaningsPerFold])	
	...
	for meaning in words:
		...
		if meaning in testMeanings:
			testWords = words[meaning]
			trainWords = []
		else:
			testWords = []
			trainWords = words[meaning]		
	...
	pickle.dump([X_train, y_train, X_test, y_test], open('ConceptDataFold'+str(fold+1)+'.pkl', 'w'))	
