...
def feature_for_word_pair(pair):
	...
	if not use_new_model:
		...
		## New Features
		fac_a = float(len(pair[0]))/(len(pair[0]) + len(pair[1]))
		fac_b = float(len(pair[1]))/(len(pair[0]) + len(pair[1]))
		for subseq in PHI[pair[0]]:
			if subseq in PHI[pair[1]]:
				feature[subseq_id[subseq]] = fac_a*PHI[pair[0]][subseq] + fac_b*PHI[pair[1]][subseq]
			else:
				feature[subseq_id[subseq]] = fac_a*PHI[pair[0]][subseq]
		for subseq in PHI[pair[1]]:
			if subseq not in PHI[pair[0]]:
				feature[subseq_id[subseq]] = fac_b*PHI[pair[1]][subseq]
